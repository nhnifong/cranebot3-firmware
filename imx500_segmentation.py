...
def create_masks(request: CompletedRequest) -> Dict[int, (np.ndarray, np.ndarray)]:
    ...
    for i in found_indices:
        ...
        contours = create_contours(overlay)
        ...
        res[i] = (overlay, contours)
    return res


def draw_masks(masks: Dict[int, (np.ndarray, np.ndarray)]):
    ...
    if masks:
        for mask, contours in masks.values():
            overlay += mask
            cv.drawContours(overlay, contours, -1, (0,0,0), 2)
