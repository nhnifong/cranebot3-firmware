...
def sphere_circle_intersection(sphere_center, sphere_radius, circle_center, circle_normal, circle_radius):
    ...
    if abs(circle_normal[2]) < 1 - 1e-9:
        ...
        u_direction = np.cross(circle_normal, _X_AXIS)
        
    # Standard np.linalg.norm used cautiously here; np.dot is faster but np.linalg.norm is safer 
    # if cross product yields zeroes.
    ...
    p2_base = circle_center + (a_param / centers_distance) * (x0 * u_direction + y0 * v_direction)

    tangent_vector = -y0 * u_direction + x0 * v_direction
    tangent_vector_norm = sqrt(np.dot(tangent_vector, tangent_vector))
    tangent_vector = tangent_vector / tangent_vector_norm if tangent_vector_norm > 1e-9 else _X_AXIS
    
    offset = h_param * tangent_vector
    return [p2_base + offset, p2_base - offset]
...
def linear_move_cost_fn(model_params, starting_time, times, observed_positions):
    predicted_positions = eval_linear_pos(times, starting_time, model_params[0:3], model_params[3:6])
