...
def to_ursina_rotation(rvec):
    """Transforms a rodrigues rotation vector into an ursina euler rotation tuple in degrees. without scipy"""
    R, _ = cv2.Rodrigues(rvec)
    sy = np.hypot(R[0, 0], R[1, 0])
    if sy > 1e-6:
        x, y, z = np.arctan2(-R[1, 2], R[2, 2]), np.arctan2(R[0, 2], sy), np.arctan2(-R[0, 1], R[0, 0])
    else:
        x, y, z = np.arctan2(R[2, 1], R[1, 1]), np.arctan2(R[0, 2], sy), 0
    return tuple(np.degrees([-x, -z, y]))
...
class Anchor(Entity):
    ...
    def input(self, key):
        if hasattr(self, "manual_spool_wp") and self.manual_spool_wp.enabled:
            ...
            if key == 'up arrow':
                ...
                self.jog_speed = 0.1
                self.ui.send_ob(jog_spool=control.JogSpool(is_gripper=False, anchor_num=self.num, speed=0))
    ...
    def reel_manual(self, metersPerSecond):
        self.wp.enabled = False
        self.ui.send_ob(jog_spool=control.JogSpool(is_gripper=False, anchor_num=self.num, speed=metersPerSecond))
...
class CamPreview(Entity):
    ...
    def update(self):
        if self.anchor and not self.floor.locked:
            if self.cam.hovered:
                ...
                # range from 0 to 1. origin is top left
                ...
                floor_pos = project_pixels_to_floor(np.array([[u, v]]), self.anchor.anchor_cam_pose, self.app.nfconfig.camera_cal)
                if len(floor_pos) == 1:
                    pos2d = floor_pos[0]
                    ...
                    for key, other_cam in self.app.cam_views.items():
                        ...
                        uv_coords = project_floor_to_pixels(floor_pos, other_cam.anchor.anchor_cam_pose, self.app.nfconfig.camera_cal)
                        if len(uv_coords) == 1:
    ...
    def set_target_points(self, item: telemetry.TargetList):
        ...
        for target in item.targets:
            ...
            # in any other cameras which are enabled, project that floor position back into their UV coords
            uv_coords = project_floor_to_pixels(floor_pos, self.anchor.anchor_cam_pose, self.app.nfconfig.camera_cal)
            if len(uv_coords) == 1:
                uv = uv_coords[0]
